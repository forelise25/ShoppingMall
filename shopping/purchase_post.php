<?
	...
	$query1 = "insert into mall_order( orderid, goods_fk, goods_price, goods_name, goods_kind, goods_count, goods_point,										   user_id, amount, volume, trans_cost, mileage_add, mileage_use, real_amount, createdate,									   buyer_name, buyer_zipno, buyer_address, buyer_phone, buyer_hphone, buyer_email,                                             buyer_jumin, recipient_name, recipient_zipno, recipient_address, recipient_phone,                                           recipient_hphone, payment_type, bank, account, deposit_date, status
									  )
								values('$trade_code','$temp_code','$temp_price','$temp_name','$temp_kind', '$temp_count',		       '$temp_point', '$member_id_fk','$amount', '$temp_count', '$trans_cost', '$mileage_add',         '$mileage_use', '$real_mny', now(), '$buyer_name', '$buyer_zipcode', '$buyer_address',          '$buyer_phone', '$buyer_hphone', '$buyer_email', '$buyer_resnumber', '$recipient_name',         '$recipient_zipcode', '$recipient_address', '$recipient_phone', '$recipient_hphone', '1',        '$bank_name', '$buyer_name', '$deposit_date', '$pay_code')";
	$result1 = mysql_query($query1, $connect);

	if(!$result1) {
		err_msg("이 메시지가 정말 안보였으면 좋겠다ㅎㅎㅎㅎㅎ");
	}
	else {
