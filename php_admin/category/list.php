...
<form method='post' action='list.php'>
			...
			<td bgcolor="#666666">
				<table width="100%" cellspacing = "1" cellpadding = "2" border="0">
					...
					for($i=0; $rows=mysql_fetch_array($result);$i++){
						$query1= "select *from products_category2 where category_code_fk = '$rows[code]'";
						$result1 = mysql_query($query1, $connect);
						$sub_count1 = mysql_num_rows($result1);
						
						$query2 = "select *from products where category_fk='$rows[code]'";
					...
					<tr bgcolor="<?=$bgcolor?>" align='center' class='hanamii'>
						<td><?=($i+1)?></td>
						<td><?=$rows[code]?></td>
						<td><? echo("<a href='sub_list.php?sub_code=$rows[code]'>$rows[name]</a>") ?></td>
						<td><?=$sub_count1?> 개</td>
						<td><?=$sub_count2?> 개</td>
						<td>
						<a href='write.php?mode=update&id=<?=$rows[id]?>'>
						수정</a>
						<a href='delete.php?id=<?=$rows[id]?>' onclick="return confirm('정말로 삭제하시겠습니까?')">삭제</a>
						</td>
					</tr>
					<?
					} //end of for   집가는거 
	...
	<table width='700' border='0' cellspacing='0' cellpadding='3'>
		<tr>
			<td	align='center'>
				<input type='button' onclick="location='write.php'" value="등록하기">
			</td>
		</tr>
	</table>
</form>
