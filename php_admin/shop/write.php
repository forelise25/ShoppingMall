...
<form name='form1'method='post' enctype='multipart/form-data' action='insert.php'>
	<table width="700" cellspacing = "0" cellpadding = "3" border="1" align='center'>
		<tr class="hanamii" >
			<td	width='30%' bgcolor='#d9d9d9' align='center'>
				상품등록 관리
			</td>
			<td width="70%" bgcolor="#FFFFFF">
				<input type="radio" name="del_chk" value="N" <?if(($mode=='insert')||($rows[del_chk]=='N'))echo("checked")?>>등록
		...
		<tr class="hanamii" >
			...
			<td	width='70%' bgcolor='#ffffff'>
				<select name='l_category_fk'>
					<option>선택하세요.</option>
				<?
				$query2= "select * from products_category2 where category_code_fk='$category_code_fk'";
		...
		<tr class="hanamii" >
			...
			<td	width='70%' bgcolor='#ffffff'>
				<input type ='text' name='size' value="<? echo($rows[size])?>">구분은 "|"로 하세요. (예:255m|592m|256m)	
			</td>
		</tr>
		<tr class="hanamii" >
			<td	width='30%' bgcolor='#d9d9d9' align='center'>
		...
		<tr class="hanamii" >
			<td	width='30%' bgcolor='#d9d9d9' align='center'>
				...
				<?if($rows[con_html]=='1') echo("checked")?>
				>HTML
				<input type = 'radio' name='con_html' value='2'
				<?if(($mode=='insert')|| ($rows[con_html]=='2')) echo("checked")?>
				>TEXT
			</td>
			<td	width='70%' bgcolor='#ffffff'>
		...
		<tr class="hanamii" >
			...
			<td	width='70%' bgcolor='#ffffff'>
				...
				<?if($rows[option1_chk]=='Y') echo("checked")?>
				>이벤트 상품
				<input type='checkbox' name='option2_chk' value='Y'
				<?if($rows[option2_chk]=='Y') echo("checked")?>
				>신상품
			</td>
		</tr>
